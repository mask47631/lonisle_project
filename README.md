...
| 项 | 地址 |
| --- | --- |
| 聊天服务器 | `server.dev.lonisle.com:443` |
| 推送服务 | `https://push.dev.lonisle.com`（服务目录/申请入列） |

- 客户端「添加服务器」手动输入上述地址即可；也可在推送服务**服务目录**页找到并一键加入
- 加入策略为**审批制**，提交加入申请后等待 Owner 审批通过
- ⚠️ 测试环境**随时删档**，请勿保存重要数据；管理凭证不对外公开
...
- **音视频话题在线人数**：话题名后实时显示房间内人数（服务器每 15s 查询 LiveKit 刷新，仅人数变化时广播），**不在房间的用户也可看到**；未配置 LiveKit 时显示 0
